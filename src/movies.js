...
function howManyMovies(myMovies) {
  const onlyDrama =  myMovies.filter(element => element.genre.includes('Drama') && element.director === 'Steven Spielberg')
...
function dramaMoviesScore(dramaMovies) {
  const avgDrama = dramaMovies.filter(element => element.genre.includes('Drama'))
  if (avgDrama.length === 0) {
    return 0;
  }
  const dramaRating = avgDrama.reduce(function (acc, cur){
    if (cur.score == undefined) {
      return acc;
    }
    return acc + cur.score;
  }, 0)
  const avgDramaRating = dramaRating/avgDrama.length;
